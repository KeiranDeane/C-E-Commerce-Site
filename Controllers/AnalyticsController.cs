...
namespace GradedUnitVersion5KeiranDeane.Controllers
{
    public class AnalyticsController : Controller
    {
        ...
        {
            var data = (from orders in db.CustomerOrders
            ...
            var allData = (from orders in db.CustomerOrders
                           group orders by orders.DateCreated into dateGroup
                           select new OrderDateGroup()
                           {
                               OrderDate = dateGroup.Key,
        ...
        public async Task<ActionResult> Details(int? id)
        {
            ...
            CustomerOrder order = await db.CustomerOrders.FindAsync(id);
            if (order == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> Create(CustomerOrder order)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(CustomerOrder order)
        {
            if (ModelState.IsValid)
            {
